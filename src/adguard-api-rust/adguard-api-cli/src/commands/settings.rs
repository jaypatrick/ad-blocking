...
async fn test_api_connection(app_config: &AppConfig) -> Result<()> {
    ...
    match test_connection(app_config).await {
        ...
        Ok(false) => {
            MenuHelper::error("Connection failed. Please check your API key and network connection.");
        }
        Err(e) => {
            MenuHelper::error(&format!("Error testing connection: {:?}", e));
        }
...
fn view_configuration(app_config: &AppConfig) -> Result<()> {
    println!();
    println!("{}", console::style("═══ Current Configuration ═══").bold().cyan());
    println!();
    println!("🌐 API URL: {}", app_config.api_url);
    println!("🔑 API Token: {}", if app_config.has_token() {
        "Configured ✓"
    } else {
        "Not configured ✗"
    });
