...
class CompilerLogger {
    ...
    static [CompilerLogger] FromEnvironment() {
        $logLevel = if ($env:ADGUARD_COMPILER_LOG_LEVEL) { $env:ADGUARD_COMPILER_LOG_LEVEL } else { 'INFO' }
        $logFile = $env:ADGUARD_COMPILER_LOG_FILE
        
        if ($logFile) {
            return [CompilerLogger]::new($logLevel, $logFile)
        }
        else {
            return [CompilerLogger]::new($logLevel)
        }
    }
