...
Project scope
- This repo houses multiple sub-projects: a TypeScript filter compiler, an AdGuard DNS API client (C#), a .NET webhook app, a small console UI for the API client, a Gatsby website, and a few automation scripts.
...
- Restore/build/test solution: cd src/webhook-app; dotnet restore Code.sln; dotnet build Code.sln; dotnet test Code.sln
- Run locally (requires webhook URL): $env:ADGUARD_WEBHOOK_URL="https://linkip.adguard-dns.com/linkip/<DEVICE_ID>/<AUTH_TOKEN>"; dotnet run
  Notes
  - Configuration also reads AdGuardWebHookUrl from appsettings.json; environment variable takes precedence for quick runs.
...
- ADGUARD_WEBHOOK_URL (required by webhook app). Set in the environment for local runs; do not commit.
...
- Webhook app (src/webhook-app/)
  - .NET 8 console that triggers an AdGuard DNS webhook (GET) to refresh device settings. Program.cs wires logging + configuration, requires ADGUARD_WEBHOOK_URL, and uses a custom DelegatingHandler (Infrastructure/ClientSideRateLimitedHandler.cs) backed by System.Threading.RateLimiting's FixedWindowRateLimiter to throttle client-side requests. Extensions/HttpResponseMessage.cs has helpers for structured logging of responses.
...
- Root README lists prerequisites: .NET 8, Node 18+, and PowerShell 7+. It also documents the webhook URL and the typical steps to compile filters with the TypeScript tool.
